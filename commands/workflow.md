...
- **e2e:e2e-explorer** — read-only context gathering (codebase, conventions, constraints, build/run/test commands).
- **e2e:e2e-planner** — turns the clarified goal + findings into a reviewable, step-by-step plan.
- **e2e:e2e-executor** — implements the approved plan; also applies fixes during the verify loop.
- **e2e:e2e-tester** — writes and runs tests; reports pass/fail and coverage.
- **e2e:e2e-reviewer** — independent verification; returns a GREEN/RED verdict.
...
- For anything touching an existing codebase, delegate to **e2e:e2e-explorer** first so your questions and options are grounded in what's actually there.
...
- If the verdict is RED:
  ...
  3. Re-run **e2e:e2e-tester**.
  4. Repeat until the reviewer returns GREEN — but cap at 3 consecutive RED verdicts. If the reviewer is still RED after the third fix attempt, stop looping and surface the situation to the user: what's still failing, what was tried, and the suspected root cause. Let the user decide whether to keep going, revise the plan, or change scope.
